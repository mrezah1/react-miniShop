...
const Order = ({ title, price, count,plus,minus,delete:deletePr }) => {
  return (
    <div className="cart-product pb-3 row m-0">
      ...
      <p className="cart-product__price col-4 col-md-2">{price}</p>
      <p className="cart-product__count col-4 col-md-2">
        <button onClick={plus} className="plusBtn custom-flex fh">+</button>
        <span className="px-3">{count}</span>
        <button onClick={minus} className="minusBtn custom-flex fh">-</button>
      </p>
